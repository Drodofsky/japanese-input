...
/// fixture on disk, so the cycle skips straight from 6 to 8, and stops at 9.
const STROKE_SAMPLES: [&str; 8] = ["一", "こ", "あ", "円", "右", "ぎ", "雨", "音"];
...
fn bench_recognize(c: &mut Criterion) {
    ...
    let corpus: Vec<Vec<Vec<(f32, f32)>>> =
        STROKE_SAMPLES.iter().map(|&name| load_test_file(name)).collect();

    let mut next = 0_usize;
    c.bench_function("recognize (one drawing per round, cycling stroke counts 1-9)", |b| {
        b.iter(|| {
