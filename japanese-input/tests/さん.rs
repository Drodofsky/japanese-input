...
fn correct() {
    ...
    let reference = load_kanji_node(&map, '三');
...
fn wo1() {
    ...
    let result = match_strokes(reference, &user);
    assert_eq!(result[0].user_stroke_order.as_slice(), vec![2, 0, 1]);
}
...
fn wo2() {
    ...
    assert_eq!(result[0].user_stroke_order.as_slice(), vec![2, 1, 0]);
}
#[test]
fn wo3() {
    let map = load_kanji_map();
    ...
    assert_eq!(result[0].user_stroke_order.as_slice(), vec![0, 2, 1]);
}
#[test]
fn p1() {
    let map = load_kanji_map();
...
fn p1_wo() {
    ...
    assert_eq!(result[0].user_stroke_order.as_slice(), vec![0, 3, 2]);
}
#[test]
fn p2() {
    let map = load_kanji_map();
    ...
    assert_eq!(result[0].user_stroke_order.as_slice(), vec![0, 2, 3]);
}
#[test]
fn m1() {
    let map = load_kanji_map();
