...
fn wo1() {
    ...
    let result = match_node(&reference, &user);
    assert_eq!(result[0].user_strokes, vec![2, 0, 1]);
}
...
fn wo3() {
    ...
    assert_eq!(result[0].user_strokes, vec![0, 2, 1]);
}
#[test]
fn p1() {
    let map = load_kanji_map();
...
fn p1_wo() {
    ...
    assert_eq!(result[0].user_strokes, vec![0, 3, 2]);
}
#[test]
fn p2() {
    let map = load_kanji_map();
    ...
    assert_eq!(result[0].user_strokes, vec![0, 2, 3]);
}
#[test]
fn m1() {
    let map = load_kanji_map();
...
fn m2() {
    ...
    assert_eq!(result[0].user_strokes, vec![usize::MAX, usize::MAX, 0]);
}
